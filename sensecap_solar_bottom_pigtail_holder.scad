...
pigtail_front_width = 5.6+0.1;
pigtail_back_width = 4.2+0.1;
...
cube([block_width, block_depth, block_height], anchor=BOTOM) {
    tag("remove") fwd(8) {
        right(0) up(2) {
        // pigtail nut trap
            fwd(-8) zrot(90) yrot(90) nut_trap_side(10, "M7", shape="hex", thickness="DIN", poke_len=15);
            // pigtail front
            down(4
            ) fwd(-10) cube([pigtail_front_width,15,15], anchor=FRONT);
            // pigtail back
            down(5) fwd(0) xrot(90) cylinder(h=20, r=pigtail_back_width, anchor=FRONT);
        }
        // top right notch
        fwd(10) up(block_height/2-notch_height/2+0.01) left(19.01) cube([notch_width + 1, 30, notch_height], anchor=FRONT);
        // top ridge
        attach(UP) left(16)down(5) cube([44, 5, 5]);              
        // USB notch
        fwd(10) 
        ...
        // screw hole right
        up(screw_hole_y) back(screw_hole_z) left(10) zrot(90) yrot(90) screw_hole("m3,20")
 ...
 //nut_trap_side(10, "m3", poke_len=10);